...
\end{enumerate}
Step~1 is usually referred to as the \defkeywd{resampling step} and Step~2 as the \defkeywd{dynamics step}. In this case, running a trajectory means applying the SSA to evolve the trajectory forward in time. However, the generality of the WE method permits the use of other time evolution algorithms for the dynamics step. In the more commonly encountered applications of the WE method, a molecular dynamics algorithm is used to evolve independent trajectories through phase space.
...
Some important conceptual issues arise when applying the weighted-ensemble method to chemical-kinetics trajectories evolved using the SSA. The WE method is only guaranteed to be statistically exact if the resampling step occurs at the same time for all the trajectories in the ensemble \cite{we-exact}. However, the SSA chooses non-uniform timesteps, so it would seem impossible to ``pause'' all the trajectories at the same time $t_\text{sync}$. 
...
\begin{equation}
    \Prob(\xi) = \sqrt{\frac{B}{2\pi}} \exp\left(-\frac{B \xi^2}{2}\right)
    \label{eq:pd-dist-xi}
\end{equation}
...
For all weighted-ensemble runs shown in this work, $N_b = 30$ bins were used and the range $[n_\text{min}, n_\text{max}]$ was chosen to encompass about four standard deviations of the distribution in either direction.
